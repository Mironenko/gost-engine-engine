...
static GOST_mac_ctx* gost_mac_placement_new(const GOST_mac* m, void* buf);
...
const GOST_mac Gost_mac_base = {
    ...
    INIT_MEMBER(placement_new, gost_mac_placement_new),
    INIT_MEMBER(free, gost_mac_free),
};

static GOST_mac_ctx* gost_mac_new(const GOST_mac *m)
{
    void *buf = OPENSSL_zalloc(sizeof(GOST_mac_ctx));
    GOST_mac_ctx *ctx = gost_mac_placement_new(m, buf);
    if (!ctx) {
        OPENSSL_free(buf);
        ctx = NULL;
    }
...
static GOST_mac_ctx* gost_mac_placement_new(const GOST_mac *m, void* buf)
{
    GOST_mac_ctx *ctx = (GOST_mac_ctx*)buf;
    if (!ctx)
        return ctx;
