...
static int magma_imit_init(GOST_mac_ctx *ctx)
{
    return omac_init(ctx, SN_magma_cbc);
}

static int grasshopper_imit_init(GOST_mac_ctx *ctx)
{
    return omac_init(ctx, SN_grasshopper_cbc);
}

static int omac_imit_update(GOST_mac_ctx *ctx, const void *data, size_t count)
{
    OMAC_CTX *c = GOST_mac_ctx_data(ctx);
    if (!c->key_set) {
...
static int omac_imit_final(GOST_mac_ctx *ctx, unsigned char *md)
{
    OMAC_CTX *c = GOST_mac_ctx_data(ctx);
...
static int omac_imit_copy(GOST_mac_ctx *to, const GOST_mac_ctx *from)
{
    OMAC_CTX *c_to = GOST_mac_ctx_data(to);
    const OMAC_CTX *c_from = GOST_mac_ctx_data(from);
...
/* Clean up imit ctx */
static int omac_imit_cleanup(GOST_mac_ctx *ctx)
{
    OMAC_CTX *c = GOST_mac_ctx_data(ctx);

    if (c) {
        CMAC_CTX_free(c->cmac_ctx);
        memset(GOST_mac_ctx_data(ctx), 0, sizeof(OMAC_CTX));
    }
    return 1;
}
...
static GOST_mac Gost3412_mac_omac_base = {
    INIT_MEMBER(base, &Gost_mac_base),

    INIT_MEMBER(input_blocksize, 8),
    INIT_MEMBER(flags, EVP_MD_FLAG_XOF),
    INIT_MEMBER(algctx_size, sizeof(OMAC_CTX)),

    INIT_MEMBER(update, omac_imit_update),
    INIT_MEMBER(final, omac_imit_final),
    INIT_MEMBER(copy, omac_imit_copy),
    INIT_MEMBER(cleanup, omac_imit_cleanup),
    INIT_MEMBER(ctrl, omac_imit_ctrl),
};

const GOST_mac magma_omac_mac = {
    INIT_MEMBER(base, &Gost3412_mac_omac_base),

    INIT_MEMBER(nid, NID_magma_mac),
    INIT_MEMBER(result_size, 8),

    INIT_MEMBER(init, magma_imit_init),
};

const GOST_mac grasshopper_omac_mac = {
    INIT_MEMBER(base, &Gost3412_mac_omac_base),

    INIT_MEMBER(nid, NID_grasshopper_mac),
    INIT_MEMBER(result_size, 16),

    INIT_MEMBER(init, grasshopper_imit_init),
};
